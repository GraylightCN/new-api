...
func VolcTaskDelete(c *gin.Context) {
	...
	// Native Volc tasks are persisted with Platform="volc-native" on the reused
	// VolcEngine(45) channel; guard on the platform, not a dedicated channel type.
	if task.Platform != constant.TaskPlatformVolcNative {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": fmt.Sprintf("task does not belong to a native Volc platform (platform=%s)", task.Platform),
		})
		return
	}
