...
// Adaptor is our Volc-native sync image adaptor: Volc-native image endpoints
// only. It is selected via relay_format="volc" (VolcImageHelper) on the reused
// VolcEngine(45) channel, not by a dedicated channel type. It only supports the
// /api/v3/images/generations endpoint; all other relay modes return a clear
// "not supported" error directing users to the volcengine channel for
...
// ConvertVolcRequest is a no-op pass-through called by relay/volc_handler.go's
// VolcImageHelper. The request body is already in Volc-native format.
