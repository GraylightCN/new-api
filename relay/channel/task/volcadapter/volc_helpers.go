...
func validateVolcNativeTaskRequest(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	...
	if modelRaw, ok := body["model"]; ok {
		...
		if err := common.Unmarshal(modelRaw, &modelName); err == nil && modelName != "" {
...
func hasImageOrVideoInVolcContent(contentRaw json.RawMessage) bool {
	...
	if err := common.Unmarshal(contentRaw, &items); err != nil {
		return false
	}
	for _, item := range items {
		...
		var typeStr string
		if err := common.Unmarshal(typeRaw, &typeStr); err != nil {
			continue
		}
...
func hasVideoInVolcContent(contentRaw json.RawMessage) bool {
	...
	for _, item := range items {
		...
		if ok {
			var typeStr string
			if err := common.Unmarshal(typeRaw, &typeStr); err == nil && typeStr == "video_url" {
				return true
			}
		}
